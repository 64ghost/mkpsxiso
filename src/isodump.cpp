    ...
    for(int e=2; e<entriesFound; e++) {
        ...
        if (dirEntries.dirEntryList[e].flags & 0x2) {
            ...
            if (element != NULL) {
				...
				newelement->SetAttribute("srcdir",(param::outPath.substr(0,param::outPath.length()-1) + global::isoPath).c_str());
...
void ParseISO(cd::IsoReader& reader) {
    ...
    if (!param::xmlFile.empty()) {
		...
		if (descriptor.copyrightFileIdentifier[0] != 0x20)
			newElement->SetAttribute("copyright", CleanVolumeId(descriptor.copyrightFileIdentifier));

		trackElement->InsertEndChild(newElement);

		newElement = xmldoc.NewElement("directory_tree");
		newElement->SetAttribute("srcdir",(param::outPath.substr(0,param::outPath.length()-1)).c_str());
...
int main(int argc, char *argv[]) {
    ...
    printf("isodump v0.32.210426 - PlayStation ISO dumping tool\n");
