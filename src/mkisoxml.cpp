...
int ParseDir(tinyxml2::XMLElement *dirElement, tinyxml2::XMLDocument *xmldoc,const char *path) {
...

	{
		...
		dirElement->SetAttribute("srcdir", strstr(currentDir,path));
	}
	...
	while((dir = readdir(d)) != NULL) {
		...
		if (S_ISDIR(attr.st_mode)) {
			...
			ParseDir(fileElement, xmldoc,path);
...
int main(int argc, const char* argv[]) {
	...
	if (ParseDir(dirTreeElement, &xmldoc ,path) != 0)
		return EXIT_FAILURE;
